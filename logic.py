...
class NetworkEngine:
    ...
    def get_broadcast_addresses(self):
        """Return list of broadcast IPs for local and neighboring subnets (e.g. 192.168.1.255, 192.168.0.255) for beacon sending."""
        try:
            local_ips = socket.gethostbyname_ex(socket.gethostname())[2]
        except socket.gaierror:
            return []
        ...
        for ip in ips:
            ...
            # Neighboring /24 subnets (e.g. 192.168.0.255, 192.168.2.255) so devices on adjacent subnets see us
            try:
                third = int(parts[2])
                for delta in (1, -1):
                    neighbor = third + delta
                ...
                pass
        return out[:7]  # own + up to 2 neighbors per interface, cap to avoid sending to too many
